...
import { Box, Grid, List, ListItem, Paper, Stack, Typography, useTheme } from '@mui/material'
                ...
                <List sx={{ marginTop: 3 }}>
                    {
                        journalSidebarLinks.map(item => (
                            <ListItem key={item.id} sx={{ padding: '0px', marginTop: 1 }} >
                                <Link href={item.link} style={{ ...(pathname === item.link ? { backgroundColor: main, color: 'white' } : {}), width: '100%', padding: '10px', borderRadius: '10px' }}>
